...
public:
    BFS(const std::string &file_name);
...
BFS::BFS(const std::string &file_name) {
...
void BFS::CalculateNodeBound(Node &node, Node parent_node, int city) {
    ...
    for (int i = 0; i < node.matrix.size(); i++) {
        // blokuje wiersz i kolumnę (ścieżki z ostatniego miasta i do nowego miasta)
...
int BFS::ReduceMatrix(std::vector<std::vector<int>> &matrix) {
    int cost = 0;
...
std::vector<int> BFS::FollowPath(int current_node, std::vector<bool> visited,
...
bool BFS::NN_CheckIfAllVisited(std::vector<int> &path) {
    bool n;
...
int BFS::NN_PathCost(const std::vector<int> &path) {
    int cost = 0;
    for (int i = 0; i < path.size(); i++) {
...
std::vector<int> BFS::NN_GetALLShortestNodes(int current_node, std::vector<bool> &visited) {
    int min = std::numeric_limits<int>::max();
...
int BFS::ReadTSPFile(const std::string &file_name) {
    std::ifstream file;
    file.open(file_name);
    if (file.is_open()) {
        std::string line;
...
int BFS::ReadConfigFile(const std::string &file_name) {
    std::ifstream file;
    file.open(file_name);
    if (file.is_open()) {
        std::string line;
...
void BFS::PrintPath(const std::vector<int> &path) {
    for (int node : path) {
        std::cout << node << " ";
    }
    std::cout << std::endl;
...
void BFS::PrintMatrix(std::vector<std::vector<int>> matrix) {
