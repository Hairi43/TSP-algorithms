...
public:
    DFS(const std::string &file_name);
...
DFS::DFS(const std::string &file_name) {
    if (ReadConfigFile(file_name) && ReadTSPFile(this->parameters.file_name)) {
        ...
        // odkomentować jeśli chcę zmierzyć czas osobno dla liczenia UB i algorytmu bfs
        // this->times.start = std::chrono::steady_clock::now();
...
void DFS::DepthFirstSearch() {
    Stack<Node> stack;  // implementacja w heap.h
    ...
    Node node(this->parameters.starting_city, 0, 0, path, visited);
    ...
    while (!stack.empty()) {
        ...
        // jeśli odwiedzone zostały wszystkie wierzchołki to sprawdzamy powrót do miasta
        if (node.path.size() == this->node_num && this->matrix[node.path.back()][this->parameters.starting_city] != MAX_EDGE) {
            ...
            // if(!node.visited[node.id]) {
                ...
                // od 1, bo 0 wierzchołek to ten startowy i jego nie chcemy nadpisać
                for (int i = 0; i < this->node_num; i++) {
                    if(!node.visited[i] && this->matrix[node.path.back()][i] != MAX_EDGE) {
                        Node child(i, node.cost, 0, node.path, node.visited);

                        child.cost += this->matrix[node.path.back()][i];
...
/*
void DFS::CalculateNodeBound(Node &node, Node parent_node, int city) {
...
int DFS::ReduceMatrix(std::vector<std::vector<int>> &matrix) {
    int cost = 0;
...
std::vector<int> DFS::FollowPath(int current_node, std::vector<bool> visited,
...
bool DFS::NN_CheckIfAllVisited(std::vector<int> &path) {
    bool n;
...
int DFS::NN_PathCost(const std::vector<int> &path) {
    int cost = 0;
    for (int i = 0; i < path.size(); i++) {
...
std::vector<int> DFS::NN_GetALLShortestNodes(int current_node, std::vector<bool> &visited) {
    int min = std::numeric_limits<int>::max();
...
int DFS::ReadTSPFile(const std::string &file_name) {
    std::ifstream file;
    file.open(file_name);
    if (file.is_open()) {
        std::string line;
...
int DFS::ReadConfigFile(const std::string &file_name) {
    std::ifstream file;
    file.open(file_name);
    if (file.is_open()) {
        std::string line;
...
void DFS::PrintPath(const std::vector<int> &path) {
    for (int node : path) {
        std::cout << node << " ";
    }
    std::cout << std::endl;
